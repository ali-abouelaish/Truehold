...
<script>
// Load heic2any with CDN fallback
(function(){
  ...
  input.addEventListener('change', function(){
    ...
    loadHeicLib(async function(){
      if (!window.heic2any) return; // give up silently
      ...
      for (const file of files) {
        try {
          ...
          if (isHeic) {
            // First convert HEIC -> PNG (wide support from heic2any)
            let out = await window.heic2any({ blob: file, toType: 'image/png' });
            if (Array.isArray(out)) out = out[0];
            // Then convert PNG -> WEBP via canvas
            const pngBlob = out;
