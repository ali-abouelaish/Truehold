...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            ...
            <p class="mt-2 text-sm text-gray-600">Update the payment record details for {{ $payment->agent->display_name }}.</p>
        </div>
        ...
        @if(!$payment->canBeEdited())
            <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
                <strong>Note:</strong> This payment has been marked as paid and cannot be edited.
            </div>
        @endif

        <form action="{{ route('admin.payments.update', $payment) }}" method="POST" class="bg-white shadow-lg rounded-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mt-6">
                ...
                <textarea id="notes" 
                          ...
                          {{ !$payment->canBeEdited() ? 'disabled' : '' }}>{{ old('notes', $payment->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea id="admin_notes" 
                          name="admin_notes" 
                          rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('admin_notes') border-red-500 @enderror"
                          placeholder="Enter admin notes">{{ old('admin_notes', $payment->admin_notes) }}</textarea>
                @error('admin_notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6 bg-gray-50 p-4 rounded-lg">
                <h3 class="text-lg font-medium text-gray-900 mb-2">Payment Status</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <span class="text-sm text-gray-900">{{ $payment->method_display }}</span>
                    </div>
                    @endif
                    @if($payment->paid_date)
                    <div>
                        <label class="block text-sm font-medium text-gray-500">Paid Date</label>
                        <span class="text-sm text-gray-900">{{ $payment->paid_date->format('M d, Y') }}</span>
                    </div>
                    @endif
                </div>
            </div>
